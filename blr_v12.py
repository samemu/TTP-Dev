    ...
    for opt, arg in opts:
        ...
        if opt in ("--dataset"):
            ...
            if dataset == "casas":
                ...
                # TODO: change to 6, 9, 12
                n_weeks = 3
                OBSERVS_PER_HR = 12
                data =  np.loadtxt('./data/umass_01min_allcircuits_homeA_v3.dat', delimiter=",")
                times = data[:n_weeks*7*24*OBSERVS_PER_HR,0]
                data = data[:n_weeks*7*24*OBSERVS_PER_HR,1:] # exclude time
                #global dataReadTime
                dataReadTime = datetime.now()
            ...
            elif dataset == "enocean":
                OBSERVS_PER_HR = 60
                data =  np.loadtxt('./data/Power_sensor_combined.csv', delimiter=",")
                n_dpoints = 6367
                
                #times = data[:3*7*24*OBSERVS_PER_HR,0]
                times = data[:n_dpoints,0]
                #data = data[:3*7*24*OBSERVS_PER_HR,1:]
                data = data[:n_dpoints,1:] # exclude time
                #global dataReadTime
                dataReadTime = datetime.now()
            ...
            a[autoreg:] = data[:-autoreg,0]
            data = np.concatenate((data,a[:,np.newaxis]), axis=1)
            
            (n_samples, total_features) = np.shape(data)
            print "Auto-Regression initialized for AR%d" % (autoreg)
            print "Dataset with %d time intervals loaded" % (n_samples)
